...
%		PA_Class			: Producer Accuracies of classes
%		AA						 : Average Accuracy		
...
switch lower(par)
	...
	for i=1:NumClases
	        for j=1:NumClases
	                ConfusionMatrix(i,j) = length(find(Labels==Etiquetas(i) & PreLabels==Etiquetas(j)));
	        end;
	end;
	...
	gt_t	=	sum(ConfusionMatrix,1);
	PA_Class		= diag(ConfusionMatrix) ./ gt_t';
	AA		= double(sum(PA_Class)) / double(NumClases );
