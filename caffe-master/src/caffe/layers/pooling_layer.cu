...
__global__ void MinPoolForward(const int nthreads,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    hstart = max(hstart, 0);
    wstart = max(wstart, 0);
    Dtype minval = FLT_MAX; // min
    int minidx = -1;
    const Dtype* const bottom_slice =
        bottom_data + (n * channels + c) * height * width;
    for (int h = hstart; h < hend; ++h) {
      for (int w = wstart; w < wend; ++w) {
        if (bottom_slice[h * width + w] < minval) {  // min
          minidx = h * width + w; //min
          minval = bottom_slice[minidx]; //min
        }
      }
    }
    top_data[index] = minval; //min
    if (mask) {
      mask[index] = minidx; //min
    } else {
      top_mask[index] = minidx; //min
    }
  }
}
//} zhaoys, 2016-04-26
