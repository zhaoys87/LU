...
// Usage: caffe_('layer_set_memdata', hLayer, pData, pLabel, n)
// zhaoys, Mar 28, 2016
static void layer_set_memdata(MEX_ARGS) {
  mxCHECK(nrhs == 4 && mxIsStruct(prhs[0]) && mxIsSingle(prhs[1]) && mxIsSingle(prhs[2])  && mxIsDouble(prhs[3]),
      "Usage: caffe_('layer_set_memdata', hLayer, pData, pLabel, n)");
  MemoryDataLayer<float>* memlayer = handle_to_ptr<MemoryDataLayer<float> >(prhs[0]);
  float* mat_memdata_ptr = NULL;
  float* mat_memlabel_ptr = NULL;
  mat_memdata_ptr = reinterpret_cast<float*>(mxGetData(prhs[1]));
  mxCHECK(mat_memdata_ptr != NULL, "mat_memdata_ptr is invalidate");
  printf("test: mat_memdata_ptr[0] = %.3f\n", mat_memdata_ptr[0]);
  mat_memlabel_ptr = reinterpret_cast<float*>(mxGetData(prhs[2]));
  mxCHECK(mat_memlabel_ptr != NULL, "mat_memlabel_ptr is invalidate");
  printf("test: mat_memlabel_ptr[0] = %.1f\n", mat_memlabel_ptr[0]);
  int n = static_cast<int>(mxGetScalar(prhs[3]));
  memlayer->Reset(mat_memdata_ptr, mat_memlabel_ptr, n);
}
...
static handler_registry handlers[] = {
  ...
  // My API
  // zhaoys, Mar 28, 2016
  { "layer_set_memdata", layer_set_memdata},
  // The end.
  { "END",                NULL            },
};
